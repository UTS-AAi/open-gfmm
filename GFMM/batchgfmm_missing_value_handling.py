...
from functionhelper.hyperboxadjustment import modifiedIsOverlap
...
#                self.clusters[i] = np.array([i], dtype = np.int32)
        ...
        while isTraining:
            ...
            yX, xX = self.V.shape
            labList = np.unique(self.classId[self.classId != UNLABELED_CLASS])[::-1]
            ...
            for i in range(len(labList)):
                ...
                if self.simil == 'short':
                    for j in range(clSize):
                        b[clIdxs[j], clIdxs] = memberG(np.maximum(Wi[j], Vi[j]), np.minimum(Vi[j], Wi[j]), np.minimum(Vi, Wi), np.maximum(Wi, Vi), self.gamma, self.oper)
                elif self.simil == 'long':
                    for j in range(clSize):
                        b[clIdxs[j], clIdxs] = memberG(Vi[j], Wi[j], Wi, Vi, self.gamma, self.oper)
                else:
                    for j in range(clSize):
                        b[clIdxs[j], clIdxs] = memberG(Vi[j], Wi[j], np.minimum(Vi, Wi), np.maximum(Wi, Vi), self.gamma, self.oper)
...
#                newW[tmp_row] = np.maximum(self.W[row1], self.W[row2])
                ...
                if ((((newW[int(curmaxb[0])] - newV[int(curmaxb[0])]) <= self.teta).all() == True) and (not modifiedIsOverlap(newV, newW, int(curmaxb[0]), newClassId))):
                    isTraining = True
                    self.V = newV
                    self.W = newW
