...
from functionhelper.hyperboxadjustment import modifiedIsOverlap, overlapTestOneMany
...
#                newW[tmp_row] = np.maximum(self.W[row1], self.W[row2])
                ...
                #if ((((newW[int(curmaxb[0])] - newV[int(curmaxb[0])]) <= self.teta).all() == True) and (not overlapTestOneMany(newV, newW, int(curmaxb[0]), newClassId))):
                    isTraining = True
                    self.V = newV
                    self.W = newW
