            ...
            for key in chunk_data:
                ...
                if num_samples >= nprocs:
                    ...
                    for i in range(nprocs):
                        ...
                        patClassId = np.where(np.isnan(patClassId), UNLABELED_CLASS, patClassId)
...
#            self.no_pat = self.no_pat[index_Kept]
            ...
            while True:
                ...
                if chunk_data != None:
                    ...
                    if XlVal is None:
                        ...
                        XlVal = np.vstack((XlVal, X_l))
                        XuVal = np.vstack((XuVal, X_u))
    ...
    def granular_phase_one_classifier(self, dataFilePath, chunk_size, type_chunk = 1, isPruning = False, valFile_Path = '', accuracyPerBox = 0.5, XlT = None, XuT = None, patClassIdTest = None, file_object_save = None):
        ...
        if (not valFile_Path) == False:
            ...
            Xval_lo = Xval.copy()
            Xval_up = Xval.copy()
            Xval_lo = np.where(np.isnan(Xval_lo), 1, Xval_lo)
            Xval_up = np.where(np.isnan(Xval_up), 0, Xval_up)
            patClassIdVal_cpy = patClassIdVal.copy()
            patClassIdVal_cpy = np.where(np.isnan(patClassIdVal_cpy), UNLABELED_CLASS, patClassIdVal_cpy)
        ...
        if (XlT is not None) and (len(self.classId) > 0):
            ...
            if (not valFile_Path) == False:
                ...
                result_val = self.predict_test(Xval_lo, Xval_up, patClassIdVal_cpy)
    ...
    def predict_test(self, XlT, XuT, patClassIdTest):
        ...
        # classifications
        for i in range(yX):
            mem = memberG(XlT[i, :], XuT[i, :], np.minimum(self.V, self.W), np.maximum(self.W, self.V), self.gamma, self.oper) # calculate memberships for all hyperboxes
...
if __name__ == '__main__': 
    ...
    for exe_time in range(1):
        ...
        if len(classifier.classId) > 0:
            classifier.granular_phase_two_classifier(Xtest_lo, Xtest_up, patClassIdTest, validation_file, file_object_save)          
        else:
